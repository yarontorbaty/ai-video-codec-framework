...
def generate_blog_html(experiments, reasoning_items, total_count):
    ...
    if not blog_posts:
        ...
        for i, post in enumerate(blog_posts):
            ...
            # We'll show them with proper context labels in the UI
            ...
            <div class="blog-post" id="exp-{i+1}">
                ...
                {f'<div class="blog-section" style="background: #fff8e1; border-left: 4px solid #ffa726;"><h3><i class="fas fa-search"></i> Pre-Experiment Analysis</h3><p style="font-size: 0.9em; color: #666; font-style: italic; margin-bottom: 15px;"><i class="fas fa-info-circle"></i> This analysis was generated BEFORE the experiment ran, based on analyzing previous experiments. It explains what problems were identified and what this experiment attempted to fix.</p><h4 style="margin-top: 20px; color: #f57c00;">Root Cause from Previous Experiments:</h4><p>{root_cause}</p></div>' if root_cause else ''}
                
                {f'<div class="blog-section" style="background: #fff8e1; border-left: 4px solid #ffa726;"><h3><i class="fas fa-lightbulb"></i> Insights from Previous Experiments</h3><p style="font-size: 0.9em; color: #666; font-style: italic; margin-bottom: 15px;"><i class="fas fa-info-circle"></i> These insights were generated by analyzing past experiments to inform the design of this experiment.</p><ul>{insights_html}</ul></div>' if insights_html else ''}
