...
class AdaptiveCodecAgent:
        ...
        if commit_to_github:
            try:
                ...
                if result['commit_success'] and result['push_success']:
                    logger.info(f"✅ Committed to GitHub: {result['commit_hash'][:8]}")
                    github_info['github_committed'] = True
                    github_info['github_commit_hash'] = result.get('commit_hash')
                else:
                    logger.warning(f"⚠️  GitHub commit failed: {result.get('error')}")
                    
            except Exception as e:
                logger.warning(f"⚠️  Could not commit to GitHub: {e}")
    ...
    def should_adopt_new_code(self, new_metrics: Dict) -> bool:
        ...
        if not self.performance_history:
            ...
            if new_bitrate < HEVC_BASELINE_MBPS * 0.9:  # 10% better than HEVC
                logger.info(f"🎯 First version beats HEVC baseline: {new_bitrate:.2f} < {HEVC_BASELINE_MBPS} Mbps")
                return True
            else:
                logger.info(f"⏸️  First version not better than HEVC: {new_bitrate:.2f} vs {HEVC_BASELINE_MBPS} Mbps")
        ...
        if new_bitrate < prev_bitrate * 0.9:  # 10% better bitrate
            logger.info(f"🎯 New code is {(1 - new_bitrate/prev_bitrate)*100:.1f}% better than v{self.implementation_version}!")
            return True
        
        if new_quality > prev_quality * 1.2:  # 20% better compression ratio
            logger.info(f"🎯 New code has {(new_quality/prev_quality - 1)*100:.1f}% better compression than v{self.implementation_version}!")
            return True
        
        logger.info(f"⏸️  New code not better: {new_bitrate:.2f} vs {prev_bitrate:.2f} Mbps")
        return False
    ...
    def test_generated_code(self, code: str, function_name: str = 'compress_video_frame') -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            logger.info(f"📝 Saved code version to {version_file}")
