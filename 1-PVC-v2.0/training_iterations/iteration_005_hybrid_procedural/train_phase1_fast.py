#!/usr/bin/env python3
"""
Phase 1 Fast Training: Pre-generate dataset, suppress warnings, train fast

Key changes:
1. Pre-generate all frames before training starts
2. Suppress numpy warnings
...
# Suppress numpy warnings from rendering
...
def generate_dataset(num_samples=5000, size=(512, 960)):
    """Pre-generate all frames before training"""
    print(f"\n{'='*70}")
    print(f"PRE-GENERATING {num_samples} ANIME FRAMES")
    print(f"{'='*70}\n")
    ...
    for idx in range(num_samples):
        if idx % 100 == 0:
            print(f"Generating frame {idx}/{num_samples}...")
        ...
        # Render frame
        frame = render_anime_frame(operations, width, height)
        frame_tensor = torch.from_numpy(frame).permute(2, 0, 1).float() / 255.0
        frames.append(frame_tensor)
    
    print(f"✓ Generated {num_samples} frames!")
...
def train_phase1_fast():
    print("="*70)
    print("PHASE 1 FAST: Training Procedural Predictor for 200 Epochs")
    print("="*70)
    ...
    # PRE-GENERATE DATASET (do this once!)
    frames = generate_dataset(num_samples=5000, size=(512, 960))
    dataset = TensorDataset(frames)
    dataloader = DataLoader(dataset, batch_size=4, shuffle=True, num_workers=0)
    print(f"✓ Dataset ready: {len(dataset)} frames, batch_size=4")
    ...
    print("="*70)
    print("✓ Phase 1 Fast Training Complete!")
